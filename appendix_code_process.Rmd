...
step.all = step(lm(target_death_rate ~ ., data = cancer_all))
summary(step.all)
```
...
```{r}
set.seed(1)
Y <- cancer_all$target_death_rate
X <- model.matrix( ~ . , data = cancer_all[,-1])
train <- sample(1:nrow(X), nrow(X)/2)
grid <- 10^seq(5, -2, length = 100)
...
kfold_cv = lapply(1:10, function(i){
  ...
  model_caret <- train((target_death_rate ~ incidence_rate + med_income + median_age_male + avg_household_size + pct_no_hs18_24 + pct_hs18_24 + pct_unemployed16_over + log_ann_count + log_deaths_yr + log_pop + pct_minority + pct_healthinsurance),
                     data = cancer_all,
